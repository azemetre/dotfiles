...
function M.diagnostic(winid, nr, keep)
	fn.CocActionAsync("diagnosticList", "", function(err, res)
		if err == vim.NIL then
			...
			fn.setqflist({}, action, { id = id ~= 0 and id or nil, title = "CocDiagnosticList", items = items })
...
function M.scroll_insert(right)
	if
		...
		and fn.pumvisible() == 0
		and api.nvim_get_current_win() ~= vim.g.coc_last_float_win
	then
...
function M.initialize()
	...
	map("n", "<C-f>", [[v:lua.require'plugs.coc'.scroll(v:true)]], { noremap = true, expr = true, silent = true })
	map("n", "<C-b>", [[v:lua.require'plugs.coc'.scroll(v:false)]], { noremap = true, expr = true, silent = true })
	map("s", "<C-f>", [[v:lua.require'plugs.coc'.scroll(v:true)]], { noremap = true, expr = true, silent = true })
	map("s", "<C-b>", [[v:lua.require'plugs.coc'.scroll(v:false)]], { noremap = true, expr = true, silent = true })
	map(
		"i",
		"<C-f>",
		[[v:lua.require'plugs.coc'.scroll_insert(v:true)]],
		{ noremap = true, expr = true, silent = true }
	)
	map(
		"i",
		"<C-b>",
		[[v:lua.require'plugs.coc'.scroll_insert(v:false)]],
		{ noremap = true, expr = true, silent = true }
	)
	...
	map("n", "gt", [[<Cmd>call CocActionAsync('jumpTypeDefinition', 'drop')<CR>]])
