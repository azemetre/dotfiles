...
function M.get_signs(buf, lnum)
	...
	if vim.fn.has("nvim-0.10") == 0 then
		...
		for _, sign in
			ipairs(
				vim.fn.sign_getplaced(buf, { group = "*", lnum = lnum })[1].signs
			)
		do
...
function M.foldtext()
	...
	table.insert(ret, { " " .. Azemetre.config.icons.misc.dots })
...
function M.statuscolumn()
	...
	local show_open_folds = vim.g.azemetre_statuscolumn
		and vim.g.azemetre_statuscolumn.folds_open
	local use_githl = vim.g.azemetre_statuscolumn
		and vim.g.azemetre_statuscolumn.folds_githl
	...
	if show_signs then
		...
		for _, s in ipairs(signs) do
			if
				s.name and (s.name:find("GitSign") or s.name:find("MiniDiffSign"))
			then
				right = s
				if use_githl then
					githl = s["texthl"]
		...
		vim.api.nvim_win_call(win, function()
			...
			elseif
				show_open_folds
				and not Azemetre.ui.skip_foldexpr[buf]
