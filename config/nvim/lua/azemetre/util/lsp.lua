...
function M.rename_file()
	...
	local old = assert(Azemetre.root.realpath(vim.api.nvim_buf_get_name(buf)))
	local root =
		assert(Azemetre.root.realpath(Azemetre.root.get({ normalize = true })))
...
function M.formatter(opts)
	...
	local ret = {
		...
		priority = 1,
		format = function(buf)
			M.format(Azemetre.merge({}, filter, { bufnr = buf }))
		end,
		sources = function(buf)
			local clients =
				M.get_clients(Azemetre.merge({}, filter, { bufnr = buf }))
			---@param client vim.lsp.Client
	...
	return Azemetre.merge(ret, opts) --[[@as LazyFormatter]]
end
...
function M.format(opts)
	opts = vim.tbl_deep_extend(
		...
		{},
		opts or {},
		Azemetre.opts("nvim-lspconfig").format or {},
		Azemetre.opts("conform.nvim").format or {}
	)
		...
		vim.api.nvim_create_autocmd(
			{ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" },
			{
				...
				callback = function(ev)
					if
						not require("azemetre.plugins.lsp.keymaps").has(
							buf,
							"documentHighlight"
						)
					then
					...
					if not ({ M.words.get() })[2] then
						if ev.event:find("CursorMoved") then
							vim.lsp.buf.clear_references()
						elseif not Azemetre.cmp.visible() then
...
function M.words.get()
	...
	for _, extmark in
		ipairs(
			vim.api.nvim_buf_get_extmarks(0, M.words.ns, 0, -1, { details = true })
		)
	do
		local w = {
