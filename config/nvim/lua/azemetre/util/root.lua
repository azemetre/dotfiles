...
function M.detectors.lsp(buf)
	...
	for _, client in pairs(Azemetre.lsp.get_clients({ bufnr = buf })) do
...
function M.info()
	...
	lines[#lines + 1] = "```"
	Azemetre.info(lines, { title = "Azemetre Roots" })
...
function M.setup()
	vim.api.nvim_create_user_command("LazyRoot", function()
		Azemetre.root.info()
	end, { desc = "Azemetre roots for the current buffer" })
...
function M.get(opts)
	...
	return Azemetre.is_win() and ret:gsub("/", "\\") or ret
end
